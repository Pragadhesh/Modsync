...
import type { Case, CaseFlag, CaseStatus, RedditPreviewData } from '../../shared/api';
...
export const CaseDetailModal = ({ case: c, username, mods, onUpdate, onAddNote, onDelete, onClose }: Props) => {
  ...
  // Reddit preview
  const [parsedLink, setParsedLink] = useState(() => parseRedditUrl(c.linkedUrl ?? ''));
  ...
  // Pre-fetch preview for existing linkedUrl on open
  useEffect(() => {
    if (!c.linkedUrl) return;
    const parsed = parseRedditUrl(c.linkedUrl);
    if (!parsed) return;
    void (async () => {
      setPreviewLoading(true);
      try {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
          ...
          <div>
            ...
            {/* Preview card */}
            {parsedLink && (
              <div className={`rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60 overflow-hidden ${editingUrl ? 'mt-2' : ''}`}>
                {previewLoading ? (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        ...
                        {preview?.author && (
                          <span className="text-[10px] text-gray-400 dark:text-gray-500">· u/{preview.author}</span>
                        )}
                        {preview?.flair && (
                        ...
                        {parsedLink.type === 'comment' && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">comment</span>
                        )}
                      </div>
                    </div>
